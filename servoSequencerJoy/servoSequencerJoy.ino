...
void loadSequence()
{
   for(int j = 0; j < NUM_SERVOS; j++)
      {
        if(sequencePositions[currentSequence][j] == 0)
        {
          Serial.println("ERROR");
          return;
        ...
        else 
        {
          ...
          Serial.print(j);
          Serial.print(":");
          Serial.print(sequencePositions[currentSequence][j] );
          servoValues[j] = sequencePositions[currentSequence][j];
        }
...
void readAnalogs()
{
  for(int i = 0; i< NUM_SERVOS; i++)
  {
   ...
   if(analogVal > DEADBANDHIGH || analogVal < DEADBANDLOW)
   {
     valueMapped = map(analogVal, 0, 1023, -speed, speed); //Map analog value from native joystick value (0 to 1023) to incremental change (-speed to speed)
      servoValues[i] =  servoValues[i] - valueMapped; //add mapped shoulder joystick value to present Shoulder Value (positive values of joyShoulderMapped will increase the position, negative values will decrease the position)
   }
